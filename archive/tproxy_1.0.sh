...
delete_singbox_jump_rule() {
		nft delete rule inet fw4 mangle_prerouting handle $(nft --handle list chain inet fw4 mangle_prerouting | awk '/jump prerouting_sing_box/ {print $(NF)}') >/dev/null 2>&1
		nft delete rule inet fw4 mangle_output handle $(nft --handle list chain inet fw4 mangle_output | awk '/jump output_sing_box/ {print $(NF)}') >/dev/null 2>&1
}
